...
namespace WPA06Service
{
    class Server
    {
        ...
        public void RunServer()
        {
            ...
            try
            {
                // Set the ip address and the port to the values in the configuration
                string ip;
                string port;
        ...
        public static void Worker(Object o)
        {
            ...
            while ((i = stream.Read(bytes, 0, bytes.Length)) != 0)
            {
                ...
                else if (clientMessageParts[1] != null && clientMessageParts[1] == "Yes")
                {
                    string randomRange;
                    int randomNumber;
                    Random num = new Random();
                    ...
                    clientRange[clientMessageParts[0]] = randomRange;
                    clientCorrectNum[clientMessageParts[0]] = randomNumber.ToString();

                    //make data (which is going to be sent back to the user) equal to a string
                    data = minAndMax[0] + " " + minAndMax[1]; //Send back if it connected and the min and max ranges
                }
                else if (length > 1)
                {
                    string range;
